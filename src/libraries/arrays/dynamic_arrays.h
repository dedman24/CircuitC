...
#define VERB_array_extract_all(array, size, data, destroy)                          \
        size = VERB_array_extract_length(array);                                    \
        data = VERB_array_extract(array);                                           \
        VERB_array_destroy(array, destroy? VERB_array_free_ctx: VERB_array_keep_ctx)
...
// TO NOTE: this can go badly...
#define VERB_array_push(el, a) do{ VERB_array_push_string(&(el), sizeof(el), (a)); /* NOLINT(bugprone-sizeof-expression) */} while(0)
#define VERB_array_push_type(type, el, a)                                                                                                               \
    do{                                                                                                                                                 \
        /* name chosen so the likelihood it's the same name as 'el' is 0 (hopefully wtf is wrong with you otherwise) */                                 \
        type VERB_array_push_type_TEMPORARY123_ohAndByTheWayILikeToProgramInSBCLAllOtherLanguagesAreJustWorseReimplementationsOfIt = el;                \
        VERB_array_push(VERB_array_push_type_TEMPORARY123_ohAndByTheWayILikeToProgramInSBCLAllOtherLanguagesAreJustWorseReimplementationsOfIt, a);      \
    } while(0)

// W COMMA OPERATOR! 
#define VERB_array_pop_type(type, a) ((a)->size -= sizeof(type), *(type*)((a)->arr + (a)->size))
#define VERB_array_pop_type_memsafe(type, a) ((a)->size > sizeof(type)? ((a)->size -= sizeof(type), *(type*)((a)->arr + (a)->size)): (type)0)
...
#define VERB_array_top_abs(a) ((a)->arr + (a)->size)

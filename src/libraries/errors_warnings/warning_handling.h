...
typedef enum{
    VERB_warning_custom,
    VERB_warning_stray_preprocessor_directive,
    VERB_warning_statement,
} VERB_warning_code_t;
...
void VERB_warning_report_many(VERB_error_specifics_t* const restrict specifics, const unsigned long long line, const unsigned long long offset, const size_t count, ...){
...
void VERB_warning_report(VERB_error_specifics_t* const restrict specifics, char* const restrict msg, const VERB_warning_code_t warning_code, const unsigned long long line, const unsigned long long offset){
    switch(warning_code){
        case VERB_warning_custom:{
            VERB_warning_report_many(specifics, line, offset, 1, msg);
            break;
        }
        case VERB_warning_stray_preprocessor_directive:{
            VERB_warning_report_many(specifics, line, offset, 3, "STRAY PREPROCESSOR DIRECTIVE '", msg, "'");
            break;
        }
        case VERB_warning_statement:{
            VERB_warning_report_many(specifics, line, offset, 2, "WARNING: ", msg);
            break;
        }
        default: {

...
// name ~ names of variables with conflicting types.
void VERB_error_report_primitive_types(VERB_error_specifics_t* specifics, const unsigned long long line, const unsigned long long offset, const VERB_bytecode_t op_tok, const size_t var_cnt, VERB_variable_t* restrict* const restrict vars){
    VERB_error_report_primitive_start(specifics, line, offset);
...
//      arg num - tokeniser backend - dst type - src type   (if function call -> op == VERB_BC_call)    5 args
            ...
            if(op == VERB_BC_call)
                ...
                VERB_error_report_primitive_types(specifics, line, offset, op, va_arg(msg, size_t), va_arg(msg, VERB_variable_t* restrict*));
            break;
        }

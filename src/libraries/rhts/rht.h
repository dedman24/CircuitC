...
VERB_rht_t* VERB_rht_init(VERB_rht_t* rht){
...
// returns false on failure in statusCode variable, true otherwise.
void* VERB_rht_search_statusCode(VERB_rht_t* restrict rht, void* const restrict key, const uint32_t keylen, bool* const restrict statusCode){
    for(uint32_t r = 0; r < keylen; r++){
        VERB_rht_t* const entry = VERB_rht_get_entry(rht, key, r);

        if(entry->keylen == keylen && !memcmp(entry->key, key, keylen)){
            *statusCode = true;
            return entry->obj.ptr;
        } 
        else if(!entry->rht) break;
        rht = entry->rht;
    }

...
// bad naming scheme, not really about regexes (I KNOW WHAT REGEX IS I JUST DIDN'T KNOW WHAT TO NAME THIS).
...
// all characters in preprocessor commands.
#define VERB_REGEX_string_preprocessor "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_#"
...
// TODO: consider allowing custom numeric formats? through 'conversion' keyword, by specifying how to convert them to a binary-formatted string.
...
// assumes VERB_REGEX_string_preprocessors holds all of the extra characters in VERB_REGEX_string_numeric_second not in VERB_REGEX_string_numeric_last.
#define VERB_REGEX_string_not_statements_last  (VERB_REGEX_string_operators VERB_REGEX_string_whitespace "\n" VERB_REGEX_string_numeric_last VERB_REGEX_string_preprocessor)
...
size_t VERB_REGEX_preprocessor_length(char* const restrict string){
    return strspn(string, VERB_REGEX_string_preprocessor);
}
...
size_t VERB_REGEX_whitespace_and_newline_length(char* string){
    return strspn(string, VERB_REGEX_string_whitespace "\n");
}
...
size_t VERB_REGEX_not_newline_length(char* string){

...
typedef struct{
    ...
    size_t len;                                         // length of tokenised type.
    size_t len_fully_qualified;                         // length of fully qualified tokenised type; length of type minus the last qualifier.
    size_t len_str;                                     // length of string of type.
    uint64_t ctr;                                       // counter to track ownership.
    VERB_type_flags_t flags;                            // flags regarding type.
} VERB_type_t;
...
bool VERB_type_equal(VERB_type_t* const t0, VERB_type_t* const t1){
    return t0 == t1 || (t0->len_fully_qualified == t1->len_fully_qualified && !memcmp(t0->type, t1->type, t0->len_fully_qualified*sizeof(*t0->type))); 
}

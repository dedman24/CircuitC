...
void VERB_bytecode_op_period(char* restrict* const restrict string, VERB_tokeniser_t* const restrict tokeniser){
    if(tokeniser->bracket_depth)
        VERB_error_report(tokeniser->specifics, VERB_error_invalid_statement, tokeniser->line, tokeniser->offset, 1, "INVALID NUMBER OF OPENED AND CLOSED BRACKETS IN STATEMENT");
    VERB_op__opcode(tokeniser, VERB_BC_period);
}

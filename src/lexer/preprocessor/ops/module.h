...
#include "_includes.h"
#include "end/endmodule.h"

// modules!!! or at least I think these are modules.
// checks if specific string is in module list, otherwise skips to #endmodule.

void VERB_preprocessor_op_module(char* restrict* const restrict string, VERB_tokeniser_t* const restrict tokeniser){
    if(!VERB_preprocessor_op__check_for_whitespace(string, "#module", tokeniser)) return;

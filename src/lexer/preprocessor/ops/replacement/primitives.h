...
void* VERB_preprocessor_macro_element_init(char* const restrict definition, const size_t definition_len, char** const restrict args, size_t* const restrict arglen, const size_t argcnt, const bool isVariadic){
...
#define VERB_preprocessor_macro_init(tokeniser, name, namelen, definition, definition_len, args, arglen, argcnt, isVariadic)                                        \
    do{                                                                                                                                                             \
        VERB_preprocessor_macro_element_t* INTERNAL_def = VERB_preprocessor_macro_element_init(definition, definition_len, args, arglen, argcnt, isVariadic);       \
    \
        const bool success =                                                                                                                                        \
...
#define VERB_preprocessor_macro_0arg(tokeniser, name, namelen, contents, macrolen)                              \
    do{                                                                                                         \
        ...
        VERB_preprocessor_macro_init(tokeniser, name, namelen, def, deflen, NULL, NULL, 0, false);              \
    } while(0)

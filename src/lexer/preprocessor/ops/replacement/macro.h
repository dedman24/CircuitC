...
// inc argcnt.
    ...
    //  YES it is slowish since it's O(n) but IDC. for how few elements macros usually have (1~8) this might be the best option.
        ...
        if(*args_start != ',' && args_start + 1 != macro_start)
            VERB_error_report(tokeniser->specifics, VERB_error_invalid_statement, tokeniser->line, tokeniser->offset, 1, "INVALID ARGUMENT SEPARATOR IN #macro");
        args_start += VERB_REGEX_whitespace_length(args_start + 1) + 1;     // skips over comma and whitespace following comma. 
    }
    bool isVariadic;
    if(arguments_lengths[argcnt-1] == strlen("VERB_RESERVED_variadicMacro") && !memcmp(arguments[argcnt-1], "VERB_RESERVED_variadicMacro", strlen("VERB_RESERVED_variadicMacro"))){
        isVariadic = true;
        argcnt--;                                       // decrements argcnt so argcnt only holds the MINIMUM number of arguments REQUIRED
    }
    ...
    VERB_preprocessor_macro_init(tokeniser, name, namelen, def, deflen, arguments, arguments_lengths, argcnt, isVariadic);
}
...
// can happen if #macro is literally the last thing in the file.
    if(name[namelen] == '\0'){
        VERB_preprocessor_macro_init(tokeniser, name, namelen, NULL, 0, NULL, NULL, 0, false);
        return;
    }
...
void VERB_preprocessor_put_macro_0arg(VERB_preprocessor_macro_element_t* const restrict macro, VERB_tokeniser_t* const restrict tokeniser){
    ...
    while(*definition) VERB_lexer(&definition, tokeniser, "");
...
// #ignore specifies that macro resolution should not expand or preprocess the following statement & that it should process it directly.
    if(*len == strlen("#ignore") && memcmp(definition, "#ignore", strlen("#ignore"))){
        VERB_tokeniser_skip_whitespace(&definition, tokeniser);
...
void VERB_preprocessor_put_macro(VERB_preprocessor_macro_element_t* const restrict macro, char* restrict* const restrict string, VERB_tokeniser_t* const restrict tokeniser){
    if(macro->argcnt == 0 && !macro->isVariadic){ VERB_preprocessor_put_macro_0arg(macro, tokeniser); return; }
...
// parses call to macro.
    VERB_rht_t* correspondences = VERB_rht_init(NULL);
...
// skips all whitespace between macro name & first round bracket.
    char* const firstbracket = VERB_tokeniser__strchr(tokeniser, *string, '(');
    if(firstbracket) *string = firstbracket;
    else{
        VERB_error_report(tokeniser->specifics, VERB_error_invalid_statement, tokeniser->line, tokeniser->offset, 1, "VARIADIC MACRO NOT WITH ROUND BRACKETS");
        return;
    }

    for(size_t i = 0; i < macro->argcnt; i++){
        char* restrict characters_toskipto;
        if(i == macro->argcnt - 1){
            if(!macro->isVariadic) characters_toskipto = ")";       // non-variadic macros always end with ')'.
            else characters_toskipto = "),";                        // variadic macros can end with ')' (0 extra args) or have their arglist continue. 
        }
        else characters_toskipto = ",";                             // if not at end, then next arg is at next ','.

        char* const endarg = VERB_tokeniser__strchr_many(tokeniser, *string, characters_toskipto);
...
// allows for weird stuff to be passed as a macro argument.
        corr->len = (size_t)(endarg - *string);
        *string = endarg + 1;                                       // +1 so it skips terminating character.

        if(!VERB_rht_put(correspondences, macro->args[i], corr, free, macro->arglen[i], VERB_rht_destroy_obj)) free(corr);
    }
    char* const lastbracket = VERB_tokeniser__strchr(tokeniser, *string, ')');
    if(!lastbracket){
        VERB_error_report(tokeniser->specifics, VERB_error_invalid_statement, tokeniser->line, tokeniser->offset, 1, "MACRO DOESN'T END WITH CLOSED ROUND BRACKET ')'");
        return;
    }
// support for variadic macros (even if there are 0 args in macros -> no preceding comma, added automatically after VERB_RESERVED_variadicMacro by compiler if more than 0 args).
    if(macro->isVariadic){
        macro_correspondence_t* corr = malloc(sizeof(*corr));
        if(**string == '('){
            ...
            corr->replacement = *string - 1;                        // -1 so it includes preceding comma (string skips over it, as seen above).
            corr->len = (size_t)(lastbracket - corr->replacement);
        }
        VERB_rht_put(correspondences, "VERB_RESERVED_variadicMacro", corr, free, strlen("VERB_RESERVED_variadicMacro"), VERB_rht_destroy_obj);
    }
...
// PART 3 - cleanup & construction of new code.
    VERB_rht_destroy(correspondences, VERB_rht_free);
...
// only line has to be handled specially, everything else can be handled as normal.
    const unsigned long long old_line = tokeniser->line, old_offset = tokeniser->offset;
    while(*toanalyse) VERB_lexer(&toanalyse, tokeniser, "");
    tokeniser->line = old_line; tokeniser->offset = old_offset;

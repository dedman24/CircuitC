...
VERB_tokeniser_t* VERB_tokeniser_init(VERB_tokeniser_t* tokeniser){
    ...
    tokeniser->overloaded_operators = NULL;
...
static void VERB_token_get(char* restrict* const restrict string, VERB_tokeniser_t* const restrict tokeniser){
// do these HAVE to be called in this order? not really.
    VERB_token_fun_t fun;
// checks if it's a preprocessor directive or not.
    if(VERB_preprocessor_handle(string, tokeniser)) return;
    if(**string == '\n'){
        *string += 1;

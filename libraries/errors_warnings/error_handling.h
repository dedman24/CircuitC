...
#include "../../lexer/tokens/token_type.h"                          // for VERB_bytecode_t, VERB_type_t.
...
VERB_variable_definition_t* VERB_variable_search_byToken(VERB_tokeniser_backend_t*, VERB_variable_token_t);
...
void VERB_error_report_primitive_varg(VERB_error_specifics_t* specifics, const unsigned long long line, const unsigned long long offset, const size_t count, ...){
...
void VERB_error_report_primitive_types_function(VERB_error_specifics_t* specifics, const unsigned long long line, const unsigned long long offset, const size_t index, VERB_tokeniser_backend_t* const restrict backend, VERB_type_t* const restrict expected, VERB_type_t* const restrict actual){
    VERB_error_report_primitive_start(specifics, line, offset);
...
// pushes index of where error happened.
    ...
    VERB_array_push_string(expected->type_str, expected->len_str, &specifics->errors);
    VERB_array_push_string("' OR COMPATIBLE TYPE, PASSED '", strlen("' OR COMPATIBLE TYPE, PASSED '"), &specifics->errors);
    VERB_array_push_string(actual->type_str, actual->len_str, &specifics->errors);
    VERB_array_push_string("'\n", strlen("'\n") + 1, &specifics->errors);   // so it may push the \0 too.
}

// name ~ names of variables with conflicting types.
void VERB_error_report_primitive_types(VERB_error_specifics_t* specifics, const unsigned long long line, const unsigned long long offset, const VERB_bytecode_t op_tok, const size_t var_cnt, VERB_variable_definition_t* restrict* const restrict vars){
    VERB_error_report_primitive_start(specifics, line, offset);
    ...
    for(size_t i = 0; i < var_cnt; i++){
        VERB_array_push_string(vars[i]->type->type_str, vars[i]->type->len_str, &specifics->errors);
        if(i != var_cnt - 1)
...
void VERB_error_report(VERB_error_specifics_t* specifics, const VERB_error_code_t error_code, const unsigned long long line, const unsigned long long offset, const size_t count, ...){
    ...
    switch(error_code){
        case VERB_error_preprocessor_no_whitespace:{
            VERB_error_report_primitive_varg(specifics, line, offset, 2, "INVALID STATEMENT: NO WHITESPACES AFTER ", va_arg(msg, char*));
            break;
        }
        case VERB_error_preprocessor_no_name_after:{
            VERB_error_report_primitive_varg(specifics, line, offset, 2, "INVALID STATEMENT: NO NAME AFTER ", va_arg(msg, char*));
            break;
        }
        case VERB_error_preprocessor_include_no_file:{
            VERB_error_report_primitive_varg(specifics, line, offset, 2, "NO FILE NAMED ", va_arg(msg, char*));
            break;
        }
        case VERB_error_invalid_statement:{
            VERB_error_report_primitive_varg(specifics, line, offset, 2, "INVALID STATEMENT: ", va_arg(msg, char*));
            break;
        }
        case VERB_error_custom:{
            VERB_error_report_primitive_varg(specifics, line, offset, 1, va_arg(msg, char*));
            break;
        }
        case VERB_error_value:{
            VERB_error_report_primitive_varg(specifics, line, offset, 1, "INVALID FORMATTING OF VALUE");
            break;
        }
        case VERB_error_invalid_types:{
// op - 
//      size_t - array of variables in op                   (if normal op)                              3 args
//      arg num - tokeniser backend - dst type - src type   (if function call -> op == VERB_BC_call)    5 args
            const VERB_bytecode_t op = va_arg(msg, int);
            if(op == VERB_BC_call)
                VERB_error_report_primitive_types_function(specifics, line, offset, va_arg(msg, size_t), va_arg(msg, VERB_tokeniser_backend_t*), va_arg(msg, VERB_type_t*), va_arg(msg, VERB_type_t*));
            else 
                VERB_error_report_primitive_types(specifics, line, offset, op, va_arg(msg, size_t), va_arg(msg, VERB_variable_definition_t* restrict*));
            break;
        }
        case VERB_error_debug:{
            VERB_error_report_primitive_varg(specifics, line, offset, 2, "DEBUG ERROR: ", va_arg(msg, char*));
            break;
        }
        case VERB_error_macro_redefinition:{                // expects macro name, macro name length.
            ...
            VERB_error_report_primitive_varg(specifics, line, offset, 3, "INVALID REDEFINITION OF MACRO '", name, "'");
            name[len] = old;
            break;
        }
        case VERB_error_macro_argument_redefinition:{       // expects macro name, macro name length.
            ...
            VERB_error_report_primitive_varg(specifics, line, offset, 3, "INVALID REDEFINITION OF MACRO ARGUMENT '", name, "'");
            name[len] = old;
            break;
        }
        default: {
            ...
            VERB_error_report_primitive_varg(specifics, line, offset, 3, "ERROR HANDLING IS BROKEN; unhandled error code IS ", error_code_str, "; CONTACT THE DEVELOPERS & SEND THEM THIS ERROR MESSAGE!");
            break;
        }
    }

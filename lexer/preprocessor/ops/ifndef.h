...
VERB_token_t VERB_preprocessor_op_ifndef(char* restrict* const restrict string, VERB_tokeniser_t* const restrict tokeniser){
    if(!VERB_preprocessor_op__check_for_whitespace(string, "#ifndef", tokeniser)) return VERB_TOKEN_special_IGNORE;
// gets length of name
    ...
    tokeniser->preprocessor->if_state = VERB_rht_search(tokeniser->preprocessor->defined_things, *string, name_len) != NULL?
